...
module.exports.updatePhone = async (req, res, next) => {
  ...
  try {
    const [updatedPhoneCount, updatedPhone] = await Phones.update(body, {
      where: { id },
      raw: true,
      returning: true,
    });
...
module.exports.updateAllPhonesQuelityYear = async (req, res, next) => {
  ...
  try {
    const [updatedPhoneCount, updatedPhones] = await Phones.update(body, {
      where: { year },
      raw: true,
      returning: true,
    });
...
module.exports.avgRamAllPhones = async (req, res, next) => {
  try {
    const result = await Phones.findAll({
      raw: true,
      attributes: [
        [Phones.sequelize.fn('AVG', Phones.sequelize.col('ram')), 'avgRam'],
      ],
    });
...
module.exports.countPhonesByBrands = async (req, res, next) => {
  try {
    const result = await Phones.findAll({
      ...
      attributes: [
        'brand',
        [Phones.sequelize.fn('COUNT', Phones.sequelize.col('id')), 'count'],
      ],
      group: ['brand'],
    });
...
module.exports.getPreordersPhonesByPnonesId = async (req, res, next) => {
  ...
  try {
    const foundPhone = await Phones.findByPk(id);
